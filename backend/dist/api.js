...
app.post('/api/v1/generate-storyboard', async (req, res) => {
    ...
    if (!Object.values(storyboardTypes_1.ModuleType).includes(formData.moduleType)) {
        return res.status(400).json({
            error: constants_1.FORM_ERROR_MESSAGE,
            details: `Invalid moduleType. Must be one of: ${Object.values(storyboardTypes_1.ModuleType).join(', ')}.`
        });
    }
    if (!Object.values(storyboardTypes_1.Tone).includes(formData.tone)) {
        return res.status(400).json({
            error: constants_1.FORM_ERROR_MESSAGE,
            details: `Invalid tone. Must be one of: ${Object.values(storyboardTypes_1.Tone).join(', ')}.`
        });
    }
    if (!Object.values(storyboardTypes_1.ModuleLevel).includes(formData.moduleLevel)) {
        return res.status(400).json({
            error: constants_1.FORM_ERROR_MESSAGE,
            details: `Invalid moduleLevel. Must be one of: ${Object.values(storyboardTypes_1.ModuleLevel).join(', ')}.`
        });
    }
    if (formData.language && !Object.values(storyboardTypes_1.SupportedLanguage).includes(formData.language)) {
        return res.status(400).json({
            error: constants_1.FORM_ERROR_MESSAGE,
            details: `Invalid language. Must be one of: ${Object.values(storyboardTypes_1.SupportedLanguage).join(', ')}.`
        });
    }
    try {
