...
/**
 * Lightweight extension that mirrors Agent v2 payloads without changing runtime logic.
 * v2 agent scenes include either scene_number or scene_id plus relaxed naming symmetry.
 */
type LooseScene = StoryboardScene & {
  narrationScript?: string;
  narration_script?: string;
  onScreenText?: string;
  on_screen_text?: string;
  voiceover?: string;
  visual_ai_prompt?: string;
  ...
  imagePrompt?: string;
  instructionalTag?: InstructionalTag;
  events?: IncomingEvent[];
  audio?: { script?: string };
  screenLayout?: any;
...
function coerceInstructionalTag(tag: any, idMethod: IDMethod): InstructionalTag {
  switch (idMethod) {
    ...
    case "SAM": {
      const phase = (tag?.sam?.phase || tag?.phase || "Iterate") as SAMPhase;
      return { method: "SAM", sam: { phase } };
    }
    case "MERRILL": {
      const phase = (tag?.merrill?.phase || tag?.phase || "Application") as MerrillPhase;
      return { method: "MERRILL", merrill: { phase } };
    }
    case "GAGNE": {
      const event = (tag?.gagne?.event || tag?.event || "PresentContent") as GagneEvent;
      return { method: "GAGNE", gagne: { event } };
    }
    case "BACKWARD": {
      const stage = (tag?.backward?.stage || tag?.stage || "PlanLearning") as BackwardStage;
      return { method: "BACKWARD", backward: { stage } };
    }
    case "BLOOM": {
      const level = (tag?.bloom?.level || tag?.level || "Apply") as BloomLevel;
...
function upsertSceneVisual(scene: LooseScene, brandPalette?: string[]) {
  const s = scene;
  s.visual = s.visual || {};
  const v = s.visual;
...
function enforceScenesArray(scenes: LooseScene[], brandPalette?: string[]) {
...
export function normalizeToScenes(
  ...
  if (hasScenes(mod)) {
    const scenes = ((mod as any).scenes || []) as LooseScene[];

    // Enforce photorealism (top-level + nested)
    enforceScenesArray(scenes, brandPalette);

    // Ensure mirrors, imagePrompt and tags
    for (const s of scenes) {
      // Narration mirror from audio.script if missing
      if (!s.narrationScript && s?.audio?.script) {
        s.narrationScript = s.audio.script;
      }

      // Flatten screenLayout object to short string
      const layout = s.screenLayout;
      if (layout && typeof layout === "object") {
        s.screenLayout = layout.description || "Standard slide layout";
      }

      // Instructional tag
      s.instructionalTag = coerceInstructionalTag(s.instructionalTag, idMethod);
    ...
    const tableOfContents = scenes.map(
      (s) =>
        safeStr(s.pageTitle) ||
        safeStr(s.title) ||
        `Scene ${s.sceneNumber ?? ""}`.trim()
    );
  ...
  const scenes = pages.map<LooseScene>((p, idx) => {
    ...
    if (!scene.narrationScript && scene?.audio?.script) {
    ...
    if (layout && typeof layout === "object") {
      scene.screenLayout = layout.description || "Standard slide layout";
    }
