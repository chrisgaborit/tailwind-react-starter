...
import { Storyboard, Scene, OutcomeMap } from "./types";
import { openaiChat } from "../services/openaiGateway";
import { resetHeader } from "./resetHeader";
import { safeJSONParse } from "../utils/safeJSONParse";

/**
 * Phase 1 Enhanced: Summary Agent with Outcome Alignment
 */
export class SummaryAgent {
  async generate(storyboard: Storyboard, outcomeMap?: OutcomeMap): Promise<Scene[]> {
    let basePrompt = `
...
`;
    ...
    // Add learning outcomes context if available
    if (outcomeMap && outcomeMap.outcomes.length > 0) {
      basePrompt += `

Learning Outcomes to Recap:
${outcomeMap.outcomes.map((o, i) => `${i + 1}. ${o.outcome}`).join("\n")}

Ensure the summary explicitly recaps how each learning outcome was addressed in the module.
`;
    }
...
${outcomeMap ? '- Explicitly reference the learning outcomes achieved.' : ''}
