...
const RAG_TARGET_TABLE = process.env.RAG_TARGET_TABLE || 'storyboard_chunks_v2';
console.log(`📚 Using RAG table: ${RAG_TARGET_TABLE}`);
...
async function run() {
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      for (let j = 0; j < events.length; j++) {
        ...
        const { error: upErr } = await supabase
          .from(RAG_TARGET_TABLE)
          .upsert({
            storyboard_id: row.id,
            scene_no: i + 1,
            content: text,
            metadata: { labels, blueprint, quality_score },
            embedding: vec,
            is_archived: false
          }, { onConflict: "storyboard_id,scene_no" });
  ...
  console.log(`✅ Chunking + tagging complete. Upserted ${upserts} rows into ${RAG_TARGET_TABLE}.`);
}
