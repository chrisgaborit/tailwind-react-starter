...
// Logging hints to prevent silent misconfig
if (!GEMINI_API_KEY)
...
function sanitizeForPhotorealism(s?: string | null): string {
  // ✅ GUARD CLAUSE: If s is not a string or is empty, return a safe default.
  if (typeof s !== 'string' || !s.trim()) {
    return "A neutral, professional, high-quality image for an e-learning module, photorealistic style.";
  }
  // If it is a string, then proceed with the replacement.
  return s.replace(/vector|illustration|flat design|cartoon/gi, "photorealistic, cinematic");
}
