...
export interface SceneV2 {
  ...
  narration: string;  // Voiceover script
...
export interface InteractiveVideoOverlayOption {
  id: string;
  text: string;
  leadsTo?: string; // optional sceneIdRef or branch node id
  isCorrect?: boolean;
  coaching?: string;
}

export interface InteractiveVideoOverlay {
  atSec: number; // time within the video to show overlay
  question: string;
  options: InteractiveVideoOverlayOption[];
}

export interface InteractiveVideoSpec {
  kind: "InteractiveVideo";
  mediaUrl: string; // final video asset path
  overlays: InteractiveVideoOverlay[];
  captionsUrl?: string; // optional .vtt
}

export interface BranchHubSpec {
  kind: "BranchHub";
  options: Array<{
    id: string;
    label: string;
    leadsToNodeId: string; // BranchGraph node id
  }>;
}
...
export type InteractionKind = typeof InteractionKinds[keyof typeof InteractionKinds];
